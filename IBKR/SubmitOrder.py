...
def identifyOptionToTrade(spx_w_ExpiryStrikes, expiryTargetBusinessDaysAhead, expiryWindowIndays, spxPreviousClose, percentageChangeTargetForOptionStrike, optionType):
    ...
    targetStrike = getLargestLessThenOrEqualTo(sorted(spx_w_ExpiryStrikes[expiryDateXDaysAhead]), spxTargetLevel)
    log(f"The target expiry {expiryDateXDaysAhead} is {expiryDateXDaysAhead} days ahead and the target strike that is {percentageChangeTargetForOptionStrike} away from yesterday's close {spxPreviousClose} is {targetStrike}")
